...
<html>
			...
			<div class="span9">
					...
					<form class="form-horizontal account_main_update" action="javascript:void(0)">
						...
						<div class="row">
							...
							<div class="span1 offset2">
								<h4 id="stream_access_help" class="user_admin_help" data-placement="right" title="" class="tooltip_me">?</h4>
							</div>
						</div>
						<div class="row" style="margin-top:20px;">
							...
							<div class="span1">
								<h4 id="stream_subscription_help" class="user_admin_help" data-placement="right" title="" class="tooltip_me">?</h4>
							</div>
						</div>
						<div class="row">
							<div class="span2 offset1">
							...
							<div id="stream_adverts" class="span2 stream_status unselectable" onclick="stream_adverts()">
								<h4>On</h4>
							</div>
							<div class="span1 offset2">
								<h4 id="stream_adverts_help" class="user_admin_help" data-placement="right" title="" class="tooltip_me">?</h4>
							</div>
						...
						<?PHP if(($stream['access'] == 'private') && !empty($total_followers)) { ?>
						<div class="row">
							<div class="span7 offset1 invite_requests">
								...
								<div id="stream_invite_requests" class="collapse in">
									<div class="user_invite_request">
							  			<div class="row">
							  					<div class="span3 dark_input">
							  						<input id="follower_search" class="search" placeholder="Username" type="text" />
							  					</div>
							  			...
							  			<div class="info_list">
							  				<div class="row unselectable">
							  					...
							  					<div class="span2 collumn" onclick="order_users('status')">
							  						Status
							  					</div>
							  					<div class="span2 collumn" style="cursor:default">
							  						Action
							  					</div>
							  				</div>
							  			</div>
				...
				<div id="posts_options" class="hidden user_admin_tab">
					...
					<div class="row">
						<div class="row">
							<div class="span7 offset1 invite_requests">
								<h3 class="unselectable">Posts</h3>
								<div class="user_invite_request">
						  			<div class="row">
						  					<div class="span3 dark_input">
						  						<input id="posts_search" class="search" placeholder="Title" type="text" />
						  					</div>
						  					<div class="span2 page_counter dark_input">
						  						<div class="pull-left desc">
						  							Results<br />per<br />page
						  						</div>
						  						<input id="post_display_num" class="results_per_page" value="10" placeholder="10" type="text" maxlength="3" />
						  					</div>
						  					<div class="span1 prevnext">
						  			...
						  			<div class="info_list">
						  				<div class="row unselectable">
						  					...
						  					<div class="span1 collumn" onclick="order_posts('status')">
						  						Status
						  					</div>
	...
	// Set the default loadout for posts
	var post_list = {
		results : 	10,
		page 	: 	0,
		search  : 	'',
	...
	var ads = <?PHP echo "'".$stream['adverts']."'" ?>;
	...
	$(function () {
		...
		stream_ads_style(); // Style the ad-toggle button
		load_users();
		load_posts();
	});

	// For the follower display count
	...
	// For the follower search box
	$('#follower_search').keypress(function(e) { if(e.which != 13) { return; }
		user_list.page = 0;
		load_users();	
	});

	// For the post search box
	$('#posts_search').keypress(function(e) { if(e.which != 13) { return; }
		post_list.page = 0;
		load_posts();	
	});

	// Pagination for follower results
	...
	function load_users()
	{
		user_list.search = $('#follower_search').val(); // Update the search parameter
		user_list.results = $('#user_display_num').val(); // Update the page parameter
		if(user_list.results == null) { user_list.results = 10; }
		offset = user_list.results*user_list.page;
		$('.user_info').remove(); // Removes Previous Data
		$.post("/stream/display_user_list", { limit: user_list.results, offset: offset, order_by: user_list.order_by, ascdesc: user_list.ascdesc, search: user_list.search}).done(function(data) {
			console.log(data);
			...
			if(user_list.page*user_list.results >= user_list.total_results-user_list.results) { $('#user_next').hide(); }
	...
	function load_posts()
	{
		post_list.search = $('#posts_search').val(); // Update the search parameter
		post_list.results = $('#post_display_num').val(); // Update the page parameter
		if(post_list.results == null) { post_list.results = 10; }
		offset = post_list.results*post_list.page;
		$('.post_info').remove(); // Removes Previous Data
		$.post("/stream/display_post_list", { limit: post_list.results, offset: offset, order_by: post_list.order_by, ascdesc: post_list.ascdesc, search: post_list.search}).done(function(data) {
			console.log(data);
			$('#posts').append(data);
			$('#post_prev').show();
			$('#post_next').show();
			$.post("/stream/count_posts", {search: post_list.search}).done(function(data) {
				$('#post_results').html(data);
				post_list.total_results = data;
			});
			if(post_list.page == 0) { $('#post_prev').hide(); }
			if(post_list.page*post_list.results >= post_list.total_results-post_list.results) { $('#post_next').hide(); }
			$('#post_page').html(post_list.page);
		});
	}

	// Updates the follower's status
	function update_follower_status(ele, status, id)
	{
		$(ele).html('working...');
		$.post("/stream/update_follower_status", { status: status, follower: id}).done(function(data) {
			console.log(data);
			load_users();
		});
	}
	...
	// Updates the stream's subscription access. Toggles between 'off' and 'on'
	function stream_subscription()
	{
		$.post("/stream/update_subscription").done(function(data) { 
			update_complete(data, 'sub_access');
			stream_sub_style(); 
		});	
	}

	// Toggles if the stream's ads are on/off
	function stream_adverts()
	{
		$.post("/stream/update_adverts").done(function(data) { 
			update_complete(data, 'ads_update');
			stream_ads_style(); 
		});	
	}
	...
	{
		console.log(sub);
		$('#stream_subscription').removeClass('trans_bg light open');
		if(sub == 'off')
		{
			$('#stream_subscription').addClass('trans_bg light');
			$('#stream_subscription').html('<h4>off</h4>');
			$("#stream_subscription_help").attr('data-original-title', 'Users cannot subscribe. Existing subscriptions persist but will not be renewed.');
			sub = 'on';
		}
		else
		{
			$('#stream_subscription').addClass('open');
			$('#stream_subscription').html('<h4>on</h4>');
			$("#stream_subscription_help").attr('data-original-title', 'Users can subscribe to your stream.');
			sub = 'off';
		}
	...
	// Restyles the stream access button to the access level passed to it
	function stream_ads_style()
	{
		console.log(ads);
		$('#stream_adverts').removeClass('trans_bg light open');
		if(ads == 'off')
		{
			$('#stream_adverts').addClass('trans_bg light');
			$('#stream_adverts').html('<h4>off</h4>');
			$("#stream_adverts_help").attr('data-original-title', 'No ads will display on your page.');
			ads = 'on';
		}
		else
		{
			$('#stream_adverts').addClass('open');
			$('#stream_adverts').html('<h4>on</h4>');
			$("#stream_adverts_help").attr('data-original-title', 'Ads are enabled for your stream.');
			ads = 'off';
		}
		$('#stream_adverts_help').tooltip();
	}
	...
	// Changes the mini-pic of the follower the user is hoevering over in the follower mgmt list
	function follower_pic(id)
	{
		$('#list_preview_1').css('background', 'url(\'/resources/profile_pics/'+id+'.png\')');
	}
